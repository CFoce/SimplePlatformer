...
import com.bfg.game.*;
import com.bfg.game.level.*;

public class SceneLevelMenu implements Scene
{
	private volatile static TextTitle title = new TextTitle("Level",225,150);
	private volatile static TextTitle title1 = new TextTitle("Selection",225,225);
	...
	public void onListener(boolean touch,int x,int y) {
		if(touch) {
			...
			if(level[0].checkTouch(x,y)) {
				setNextScene(2);
			}
			if(level[1].checkTouch(x,y)) {
				setNextScene(3);
			}
			if(level[2].checkTouch(x,y)) {
				setNextScene(4);
			}
			/*for(int i = 0; i < level.length;i++) {
				if(level[i] != null) {
